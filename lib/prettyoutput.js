...
const isSerializable = typeInspect.isSerializable
 ...
 * @property {string} [emptyArrayMsg] - Rendered message on empty array
 * @property {string} [emptyObjectMsg] - Rendered message on empty object
 */
...
internals.parseOptions = opts => {
    ...
    return {
        emptyArrayMsg: opts.emptyArrayMsg || '(empty array)',
        emptyObjectMsg: opts.emptyObjectMsg || '(empty object)',
    ...
    while (stack.length > 0) {
        ...
        if (noRender) {
            ...
            _.forEachRight(input, value => {
                ...
                stack.push({
                    input: value,
                    indentation: Array.isArray(value) ? indentation : renderer.indent(indentation, options),
                    depth: depth + 1,
                })
